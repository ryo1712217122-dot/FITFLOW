...
describe('computeWeightTrendChange', () => {
    test('windowSize=1なら生の2点の差になる(移動平均が値そのもの)', () => {
        const logs = [
            { date: '2023-01-01', weight: 71.0 },
            { date: '2023-01-08', weight: 70.2 }
        ];
        assert.equal(computeWeightTrendChange(logs, 7, 1), -0.8);
    });

    test('両端の移動平均どうしの差を返す', () => {
        // 前半3日の平均 71.0、後半3日の平均 70.0 → -1.0
        const logs = [
            { date: '2023-01-01', weight: 71.2 },
            { date: '2023-01-02', weight: 70.8 },
            { date: '2023-01-03', weight: 71.0 }, // ここまでの3日平均 = 71.0
            { date: '2023-01-08', weight: 70.3 },
            { date: '2023-01-09', weight: 69.7 },
            { date: '2023-01-10', weight: 70.0 }  // 最新。3日平均 = 70.0
        ];
        assert.equal(computeWeightTrendChange(logs, 7, 3), -1.0);
    });

    test('1日だけ大きく外れた測定値の影響が生の差より小さくなる', () => {
        // 最新日だけ +1.5kg 跳ねた(水分等)ケース。生の差なら -0.2 に見えるが、
        // 移動平均どうしなら実際のトレンド(-1.0付近)に近い値が出る
        const logs = [
            { date: '2023-01-01', weight: 71.2 },
            { date: '2023-01-02', weight: 70.8 },
            { date: '2023-01-03', weight: 71.0 },
            { date: '2023-01-08', weight: 70.3 },
            { date: '2023-01-09', weight: 69.7 },
            { date: '2023-01-10', weight: 71.5 }  // 外れ値
        ];
        const raw = 71.5 - 71.0; // 生の2点差なら +0.5
        const trend = computeWeightTrendChange(logs, 7, 3);
        assert.ok(trend < raw, `移動平均差(${trend})は生の差(${raw})より小さくあるべき`);
        assert.equal(trend, -0.5); // (70.3+69.7+71.5)/3 - 71.0 = 70.5 - 71.0
    });

    test('ちょうどdays日前のログが無ければ、目標日に最も近いログを使う', () => {
        const logs = [
            { date: '2023-01-02', weight: 71.0 }, // 目標日(01-03)との差: 1日 → こちらが選ばれる
            { date: '2023-01-06', weight: 70.5 },
            { date: '2023-01-10', weight: 70.2 }  // 最新。7日前 = 01-03
        ];
        assert.equal(computeWeightTrendChange(logs, 7, 1), -0.8); // 70.2 - 71.0
    });

    test('ログが1件以下ならnullを返す', () => {
        assert.equal(computeWeightTrendChange([], 7), null);
        assert.equal(computeWeightTrendChange([{ date: '2023-01-01', weight: 70 }], 7), null);
    });
});
...
describe('addDaysToDateString', () => {
    test('日数を加算する', () => {
        assert.equal(addDaysToDateString('2026-07-01', 14), '2026-07-15');
        assert.equal(addDaysToDateString('2026-07-01', 0), '2026-07-01');
    });

    test('月またぎ・年またぎでも正しい', () => {
        assert.equal(addDaysToDateString('2026-07-25', 14), '2026-08-08');
        assert.equal(addDaysToDateString('2026-12-25', 14), '2027-01-08');
    });

    test('不正な日付は空文字', () => {
        assert.equal(addDaysToDateString('not-a-date', 5), '');
    });
});

describe('findWeightNearDate', () => {
    const logs = [
        { date: '2026-07-01', weight: 81.0 },
        { date: '2026-07-10', weight: 80.4 },
        { date: '2026-07-20', weight: 79.8 }
    ];

    test('その日ちょうどの記録があればそれを返す', () => {
        assert.equal(findWeightNearDate(logs, '2026-07-10').weight, 80.4);
    });

    test('無ければ最も近い記録を返す', () => {
        assert.equal(findWeightNearDate(logs, '2026-07-08').weight, 80.4);
        assert.equal(findWeightNearDate(logs, '2026-07-03').weight, 81.0);
    });

    test('maxGapDaysを超えて離れていればnull', () => {
        assert.equal(findWeightNearDate(logs, '2026-08-15'), null);
        assert.equal(findWeightNearDate(logs, '2026-07-10', 0).weight, 80.4);
        assert.equal(findWeightNearDate(logs, '2026-07-11', 0), null);
    });

    test('不正な行・不正な入力は無視/null', () => {
        assert.equal(findWeightNearDate([null, { date: 'x', weight: 5 }], '2026-07-10'), null);
        assert.equal(findWeightNearDate(null, '2026-07-10'), null);
        assert.equal(findWeightNearDate(logs, ''), null);
    });
});

describe('computePlanRoadmap', () => {
    // 開始 7/1 に 81.0kg、770kcal/日(=0.1kg/日)、今日は 7/29(経過28日)
    const logs = [
        { date: '2026-07-01', weight: 81.0 },
        { date: '2026-07-15', weight: 80.0 },
        { date: '2026-07-29', weight: 78.5 }
    ];

    test('計画開始日を起点に2週間ごと3ヶ月分を並べる', () => {
        const r = computePlanRoadmap('2026-07-01', 81.0, 770, logs, '2026-07-29');
        assert.deepEqual(r.map(x => x.days), [0, 14, 28, 42, 56, 70, 84, 90]);
        assert.deepEqual(r.map(x => x.date).slice(0, 3), ['2026-07-01', '2026-07-15', '2026-07-29']);
        assert.equal(r[0].label, '開始');
        assert.equal(r[r.length - 1].label, '3ヶ月');
    });

    test('計画上の予測体重を開始時体重から算出する（今日起点ではない）', () => {
        const r = computePlanRoadmap('2026-07-01', 81.0, 770, logs, '2026-07-29');
        assert.equal(r[0].planned, 81.0);
        assert.equal(r[1].planned, 79.6);   // 81.0 - 1.4
        assert.equal(r[2].planned, 78.2);   // 81.0 - 2.8
    });

    test('今日の行に現在マーカーが付く', () => {
        const r = computePlanRoadmap('2026-07-01', 81.0, 770, logs, '2026-07-29');
        const now = r.filter(x => x.isNow);
        assert.equal(now.length, 1);
        assert.equal(now[0].days, 28);
        assert.equal(now[0].label, '現在');
    });

    test('実績体重と差分（実績-計画）を併記する', () => {
        const r = computePlanRoadmap('2026-07-01', 81.0, 770, logs, '2026-07-29');
        assert.equal(r[0].actual, 81.0);
        assert.equal(r[0].diff, 0);
        assert.equal(r[1].actual, 80.0);
        assert.equal(r[1].diff, 0.4);       // 80.0 - 79.6 → 計画より重い
        assert.equal(r[2].actual, 78.5);
        assert.equal(r[2].diff, 0.3);
    });

    test('未来の節目は実績・差分がnull', () => {
        const r = computePlanRoadmap('2026-07-01', 81.0, 770, logs, '2026-07-29');
        const future = r.filter(x => x.days > 28);
        assert.ok(future.length > 0);
        assert.ok(future.every(x => x.actual === null && x.diff === null));
    });

    test('今日が刻みに乗っていなければ専用の行を差し込む', () => {
        const r = computePlanRoadmap('2026-07-01', 81.0, 770, logs, '2026-07-21'); // 経過20日
        assert.ok(r.some(x => x.days === 20 && x.isNow && x.label === '現在'));
        // 並びは日数昇順のまま
        assert.deepEqual(r.map(x => x.days), [...r.map(x => x.days)].sort((a, b) => a - b));
    });

    test('計画期間(90日)を過ぎていても今日が表に含まれる', () => {
        const r = computePlanRoadmap('2026-07-01', 81.0, 770, logs, '2026-11-01'); // 経過123日
        const last = r[r.length - 1];
        assert.ok(last.days >= 123, `最終行 ${last.days}日 が経過日数を含む`);
        assert.ok(r.some(x => x.isNow));
    });

    test('開始日が未来なら経過0日として開始行が現在になる', () => {
        const r = computePlanRoadmap('2026-08-01', 81.0, 770, logs, '2026-07-29');
        assert.equal(r[0].days, 0);
        assert.equal(r[0].isNow, true);
    });

    test('開始・1ヶ月単位・終端がisMajor', () => {
        const r = computePlanRoadmap('2026-07-01', 81.0, 770, logs, '2026-07-29');
        assert.deepEqual(r.filter(x => x.isMajor).map(x => x.days), [0, 90]);
    });

    test('不正な入力は空配列', () => {
        assert.deepEqual(computePlanRoadmap('', 81, 770, logs, '2026-07-29'), []);
        assert.deepEqual(computePlanRoadmap('2026-07-01', 0, 770, logs, '2026-07-29'), []);
        assert.deepEqual(computePlanRoadmap('not-a-date', 81, 770, logs, '2026-07-29'), []);
    });
});

describe('decidePlanStartWeightMigration', () => {
    const logs = [
        { date: '2026-07-01', weight: 78.4 },
        { date: '2026-07-10', weight: 78.0 }
    ];

    test('開始日近くの実測値と保存値が違えば書き換える', () => {
        const r = decidePlanStartWeightMigration(logs, { weightPlanStartDate: '2026-07-01', weightStart: 81.0 });
        assert.deepEqual(r, { apply: true, weightStart: 78.4, markDone: true });
    });

    test('すでに一致していれば書き換えないがフラグは立てる', () => {
        const r = decidePlanStartWeightMigration(logs, { weightPlanStartDate: '2026-07-01', weightStart: 78.4 });
        assert.deepEqual(r, { apply: false, weightStart: 78.4, markDone: true });
    });

    test('開始日の近くに記録が無ければ打ち切る', () => {
        const r = decidePlanStartWeightMigration(logs, { weightPlanStartDate: '2026-01-01', weightStart: 81.0 });
        assert.deepEqual(r, { apply: false, weightStart: null, markDone: true });
    });

    test('体重ログや開始日が無ければ次回起動で再判定させる', () => {
        assert.equal(decidePlanStartWeightMigration([], { weightPlanStartDate: '2026-07-01' }).markDone, false);
        assert.equal(decidePlanStartWeightMigration(logs, { weightPlanStartDate: null }).markDone, false);
        assert.equal(decidePlanStartWeightMigration(logs, null).markDone, false);
    });
});
...
describe('computePlannedWeightForDate', () => {
    ...
    const w0 = 80;
    const deficit = 513; // 月2kgペース相当

    test('起点日は開始体重そのもの', () => {
        assert.equal(computePlannedWeightForDate('2023-01-01', start, w0, deficit), 80);
    });

    test('kを渡さなければ従来どおりの線形外挿', () => {
        // 30日後 → 80 - 513×30/7700 = 78.0
        assert.equal(computePlannedWeightForDate('2023-01-31', start, w0, deficit), 78.0);
    });

    test('kを渡すと減速し、線形外挿より重い予測になる', () => {
        const linear = computePlannedWeightForDate('2023-04-01', start, w0, deficit);
        const decel = computePlannedWeightForDate('2023-04-01', start, w0, deficit, 37.8);
        assert.ok(decel > linear, `減速あり(${decel})は線形(${linear})より重いはず`);
        // 90日後: 線形は -6.0kg だが減速を織り込むと -4.9kg 前後
        assert.equal(linear, 74.0);
        assert.ok(decel >= 75.0 && decel <= 75.3, `90日後の予測が想定外: ${decel}`);
    });

    test('ロードマップ表(computePlanRoadmap)と同じ値になる', () => {
        // 「計画上の体重」の定義が画面ごとに分かれていた問題への回帰テスト
        const roadmap = computePlanRoadmap(start, w0, deficit, [], '2023-04-01', { kcalPerKgPerDay: 37.8 });
        roadmap.forEach(r => {
            assert.equal(
                computePlannedWeightForDate(r.date, start, w0, deficit, 37.8),
                r.planned,
                `${r.date} でグラフの予測線とロードマップ表が食い違う`
            );
        });
    });

    test('起点日より前はnull', () => {
        assert.equal(computePlannedWeightForDate('2022-12-01', start, w0, deficit), null);
    });

    test('日付・起点日・開始体重が無ければnull', () => {
        assert.equal(computePlannedWeightForDate('', start, w0, deficit), null);
        assert.equal(computePlannedWeightForDate('2023-01-01', null, w0, deficit), null);
        assert.equal(computePlannedWeightForDate('2023-01-01', start, 0, deficit), null);
    });
});
...
describe('computePlannedWeightSeries', () => {
    test('日付配列それぞれについて予測体重を並べる', () => {
        ...
        assert.deepEqual(result, [null, 80, 78.0]);
    });

    test('配列でない入力は空配列を返す', () => {
        assert.deepEqual(computePlannedWeightSeries(null, '2023-01-01', 80, 513), []);
    });
});

describe('projectWeightAfterDays / computeEquilibriumWeight', () => {
    test('kが0以下なら線形外挿にフォールバックする', () => {
        assert.equal(projectWeightAfterDays(80, 770, 30), 77.0);
        assert.equal(projectWeightAfterDays(80, 770, 30, 0), 77.0);
        assert.equal(projectWeightAfterDays(80, 770, 30, -5), 77.0);
    });

    test('減速モデルは平衡体重を下回らない', () => {
        const eq = computeEquilibriumWeight(80, 513, 37.8);
        const far = projectWeightAfterDays(80, 513, 3650, 37.8); // 10年後
        assert.ok(far >= eq, `${far} は平衡体重 ${eq} を下回ってはいけない`);
        assert.ok(Math.abs(far - eq) < 0.2, '十分な時間が経てば平衡体重へ漸近する');
    });

    test('平衡体重 = 現体重 - 赤字/k', () => {
        // 80 - 513/37.8 = 66.4
        assert.equal(computeEquilibriumWeight(80, 513, 37.8), 66.4);
    });

    test('赤字が0以下・kが不正ならnull(維持や増量では平衡体重を出さない)', () => {
        assert.equal(computeEquilibriumWeight(80, 0, 37.8), null);
        assert.equal(computeEquilibriumWeight(80, -100, 37.8), null);
        assert.equal(computeEquilibriumWeight(80, 513, 0), null);
        assert.equal(computeEquilibriumWeight(80, 513, NaN), null);
    });

    test('半減期は 7700×ln2/k 日', () => {
        assert.equal(computeHalfLifeDays(37.8), Math.round((7700 * Math.LN2) / 37.8)); // 141日
        assert.equal(computeHalfLifeDays(0), null);
    });
});
...
describe('computeActivityProfile', () => {
    ...
    const setsOf = n => [{ name: 'ベンチプレス', sets: Array.from({ length: n }, () => ({ weight: 60, reps: 10 })) }];

    test('TDEE = BMR×生活活動PAL + 有酸素日平均 + 筋トレ日平均', () => {
        const dates = Array.from({ length: 12 }, (_, i) => `2026-07-${String(19 - i).padStart(2, '0')}`);
        const workouts = dates.map((d, i) => ({ id: 'w' + i, date: d, exercises: setsOf(20) }));
        const cardio = [
            { date: '2026-07-19', distance: 4, calories: 320 },
            { date: '2026-07-12', distance: 4, calories: 340 }
        ];
        const p = computeActivityProfile(80.0, workouts, cardio, today,
            { lifestylePal: 1.55, caloriesPerSet: 15 });
        assert.equal(p.bmr, 1840);              // 23×80
        assert.equal(p.pal, 1.55);              // 筋トレ頻度ではなく設定値
        assert.equal(p.baseBurn, 2852);         // 1840×1.55
        assert.equal(p.workoutsLast30Days, 12);
        assert.equal(p.workoutBurn, 300);       // 20セット×15kcal
        assert.equal(p.workoutDailyAvg, 120);   // 12回×300 ÷ 30日
        assert.equal(p.cardioDailyAvg, 24);     // (320+340) ÷ 28日
        assert.equal(p.tdee, 2852 + 24 + 120);
    });

    test('PALは筋トレ頻度に左右されない(旧方式の二重計上の回帰テスト)', () => {
        const many = Array.from({ length: 12 }, (_, i) =>
            ({ id: 'w' + i, date: `2026-07-${String(19 - i).padStart(2, '0')}`, exercises: setsOf(20) }));
        const withWorkouts = computeActivityProfile(80.0, many, [], today, { lifestylePal: 1.55, caloriesPerSet: 15 });
        const without = computeActivityProfile(80.0, [], [], today, { lifestylePal: 1.55, caloriesPerSet: 15 });
        // ベース消費は同じ。差は筋トレの実消費(1日平均120kcal)だけで、PALによる暗黙の加算は無い
        assert.equal(withWorkouts.baseBurn, without.baseBurn);
        assert.equal(withWorkouts.tdee - without.tdee, 120);
    });

    test('記録が無ければ運動分0でTDEE=baseBurn', () => {
        const p = computeActivityProfile(70.0, [], [], today, { lifestylePal: 1.55 });
        assert.equal(p.runBurn, 0);
        assert.equal(p.runCount, 0);
        assert.equal(p.workoutDailyAvg, 0);
        assert.equal(p.cardioDailyAvg, 0);
        assert.equal(p.tdee, p.baseBurn);
    });

    test('kcalPerKgPerDay = BMR係数×PAL + 1日あたり走行km', () => {
        // 28日で56km → 2km/日。23×1.55 + 2 = 37.65
        const cardio = Array.from({ length: 28 }, (_, i) => ({
            date: `2026-0${i < 20 ? '7' : '6'}-${String(i < 20 ? 20 - i : 50 - i).padStart(2, '0')}`,
            distance: 2, calories: 160
        })).filter(c => c.date >= '2026-06-23');
        const p = computeActivityProfile(80.0, [], cardio, today, { lifestylePal: 1.55 });
        const expected = 23 * 1.55 + p.cardioKmPerDay;
        assert.ok(Math.abs(p.kcalPerKgPerDay - expected) < 1e-9);
        assert.ok(p.kcalPerKgPerDay > 23 * 1.55, '有酸素があればBMR分より大きくなる');
    });

    test('直近N日は今日を含めてN日ちょうど(以前はN+1日分を集めてNで割っていた)', () => {
        // today=2026-07-20 なら、筋トレの窓は 06-21〜07-20。06-20はぎりぎり範囲外
        const inRange = computeActivityProfile(70.0, workoutsAt(['2026-06-21']), [], today);
        const outOfRange = computeActivityProfile(70.0, workoutsAt(['2026-06-20']), [], today);
        assert.equal(inRange.workoutsLast30Days, 1);
        assert.equal(outOfRange.workoutsLast30Days, 0);
    });
...
describe('computeMeasuredTdee', () => {
    ...
    test('平均摂取 + 7700×減少ペースで実測TDEEを返す', () => {
        ...
        const r = computeMeasuredTdee(linearWeights(), meals(14), TODAY);
        assert.equal(r.tdee, 2985);
        assert.equal(r.avgIntake, 2600);
        assert.equal(r.slopeKgPerDay, -0.05);
        assert.equal(r.mealDays, 14);
        assert.equal(r.weightPoints, 28);
        // 完全な直線なので残差0 → 標準誤差0、区間は点推定に一致する
        assert.equal(r.tdeeStdError, 0);
        assert.equal(r.tdeeLow, 2985);
        assert.equal(r.tdeeHigh, 2985);
        assert.equal(r.mealCoverage, 0.52); // 14日 / 27日(今日を除いた窓)
        assert.equal(r.intakeWindowDays, 27);
    });

    test('今日の食事記録は平均摂取に含めない(未完成の1日が平均を押し下げるため)', () => {
        // 昨日までの14日は2600kcal、今日だけ食べかけの200kcal
        const withToday = meals(14).concat([
            { date: TODAY, breakfast: 200, lunch: 0, dinner: 0, snacks: 0 }
        ]);
        const r = computeMeasuredTdee(linearWeights(), withToday, TODAY);
        assert.equal(r.avgIntake, 2600, '今日の200kcalが平均に混ざってはいけない');
        assert.equal(r.mealDays, 14, '今日は記録日数にも数えない');
        // 今日を含めていたら (2600×14 + 200)/15 = 2440 になり、TDEEが160kcal低く出る
        assert.equal(r.tdee, 2985);
    });

    test('今日を含めない分、記録率の分母も1日短くなる', () => {
        // 昨日までの27日すべてに記録があれば記録率100%(分母に今日は入らない)
        const everyDayUntilYesterday = [];
        for (let i = 1; i <= 27; i++) {
            everyDayUntilYesterday.push({
                date: `2026-07-${String(i).padStart(2, '0')}`,
                breakfast: 2600, lunch: 0, dinner: 0, snacks: 0
            });
        }
        const r = computeMeasuredTdee(linearWeights(), everyDayUntilYesterday, TODAY);
        assert.equal(r.mealCoverage, 1);
        assert.equal(r.mealDays, 27);
    });

    // 体重にノイズを載せた版。実データはこちらに近い
    function noisyWeights(start = 80.0, slope = -0.05, amplitude = 0.4) {
        return linearWeights(start, slope).map((w, i) => ({
            date: w.date,
            // 決定的な擬似ノイズ(テストを再現可能にするため乱数は使わない)
            weight: Math.round((w.weight + amplitude * Math.sin(i * 1.7)) * 1000) / 1000
        }));
    }

    test('ノイズがあると標準誤差が立ち、95%区間が点推定の両側に広がる', () => {
        const r = computeMeasuredTdee(noisyWeights(), meals(14), TODAY);
        assert.ok(r, '毎日28日測っていれば精度基準を満たすはず');
        assert.ok(r.tdeeStdError > 0, `標準誤差が0のまま: ${r.tdeeStdError}`);
        assert.ok(r.tdeeLow < r.tdee && r.tdee < r.tdeeHigh);
        assert.equal(r.tdeeHigh - r.tdeeLow, Math.round(1.96 * r.tdeeStdError) * 2);
    });

    test('測定点がまばらだと標準誤差が閾値を超えてnull(件数ではなく精度で判定する)', () => {
        // 旧基準(8点・14日以上)は満たすが、ノイズに対して点が少なく傾きが定まらないケース
        const sparse = noisyWeights().filter((_, i) => i % 3 === 0); // 10点・span27日
        assert.ok(sparse.length >= 8);
        assert.equal(computeMeasuredTdee(sparse, meals(14), TODAY, { maxTdeeStdError: 50 }), null);
        // 精度の要求を緩めれば同じデータでも返る = 落ちている理由が精度であることの確認
        assert.ok(computeMeasuredTdee(sparse, meals(14), TODAY, { maxTdeeStdError: 100000 }));
    });

    test('窓内の食事記録率が低すぎるとnull(記録した日だけの平均は偏りやすい)', () => {
        // 10日分は絶対数の下限(10日)を満たすが、28日中10日=36%で記録率の下限0.5に届かない
        assert.equal(computeMeasuredTdee(linearWeights(), meals(10), TODAY), null);
        assert.ok(computeMeasuredTdee(linearWeights(), meals(14), TODAY));
    });

    test('増量期は摂取より低い実測TDEEを返す(符号そのまま)', () => {
        // 摂取3000・毎日+0.05kg → 3000 - 385 = 2615
        const r = computeMeasuredTdee(linearWeights(75.0, 0.05), meals(14, 3000), TODAY);
    ...
    test('体重測定点が3点未満なら標準誤差が定義できずnull', () => {
        const two = [linearWeights()[0], linearWeights()[27]];
        assert.equal(computeMeasuredTdee(two, meals(14), TODAY, { minWeightPoints: 2 }), null);
    });

    test('測定期間が短すぎる(最初と最後がminSpanDays未満)はnull', () => {
        const clustered = linearWeights().slice(0, 6); // 7/1〜7/6の6点(span=5日 < 既定7日)
        assert.equal(computeMeasuredTdee(clustered, meals(14), TODAY), null);
    });

    test('窓(28日)より古い記録は無視される', () => {
        const old = [{ date: '2026-05-01', weight: 99.0 }, { date: '2026-06-30', weight: 95.0 }];
        const r = computeMeasuredTdee(old.concat(linearWeights()), meals(14), TODAY);
        assert.equal(r.tdee, 2985); // 古い点が回帰に混ざれば2985にはならない
    });

    test('結果が生理的にあり得ない値ならnull(推定式へのフォールバック用)', () => {
        // 摂取500kcal/日 → 500 + 385 = 885 < 下限1000
        assert.equal(computeMeasuredTdee(linearWeights(), meals(14, 500), TODAY), null);
    });

    test('不正入力・日付はnull', () => {
        assert.equal(computeMeasuredTdee(null, meals(14), TODAY), null);
        assert.equal(computeMeasuredTdee(linearWeights(), null, TODAY), null);
        assert.equal(computeMeasuredTdee(linearWeights(), meals(14), ''), null);
        assert.equal(computeMeasuredTdee(linearWeights(), meals(14), 'invalid'), null);
    });
});
...
describe('computeMeasuredTdee: 重複日付の防御', () => {
    test('同一日付の食事重複行は先頭のみ採用し、日数閾値と平均を歪めない', () => {
        ...
        // ユニーク14日+一部重複でも、平均は先頭行(2600)のみで計算され2985になる
        const meals12 = [];
        for (let j = 1; j <= 14; j++) {
            const d = String(j).padStart(2, '0');
...
describe('computeSleepDuration', () => {
    test('日付をまたぐ睡眠を正しく計算する', () => {
        assert.equal(computeSleepDuration('23:30', '06:15'), 6.75);
        assert.equal(computeSleepDuration('00:30', '07:00'), 6.5);
    });

    test('日付をまたがない睡眠(深夜就寝)も計算できる', () => {
        assert.equal(computeSleepDuration('01:00', '08:00'), 7);
    });

    test('昼寝のような同日内の睡眠も扱える', () => {
        assert.equal(computeSleepDuration('13:00', '14:30'), 1.5);
    });

    test('就寝と起床が同時刻なら不正扱い(24時間になってしまうため)', () => {
        assert.equal(computeSleepDuration('23:00', '23:00'), null);
    });

    test('不正な時刻はnull', () => {
        assert.equal(computeSleepDuration('25:00', '07:00'), null);
        assert.equal(computeSleepDuration('23:70', '07:00'), null);
        assert.equal(computeSleepDuration('2300', '07:00'), null);
        assert.equal(computeSleepDuration('', '07:00'), null);
        assert.equal(computeSleepDuration(null, '07:00'), null);
    });
});

describe('bedTimeToAnchoredMinutes', () => {
    test('日付をまたぐ就寝時刻が連続した値になる(循環量の扱い)', () => {
        // 23:30 と 00:30 は実際には1時間差。素の分数(1410 vs 30)では1380分差になってしまう
        const a = bedTimeToAnchoredMinutes('23:30');
        const b = bedTimeToAnchoredMinutes('00:30');
        assert.equal(b - a, 60);
    });

    test('18:00が原点', () => {
        assert.equal(bedTimeToAnchoredMinutes('18:00'), 0);
        assert.equal(bedTimeToAnchoredMinutes('19:00'), 60);
    });
});

describe('filterValidSleepLogs', () => {
    test('正常な行はそのまま通す', () => {
        const r = filterValidSleepLogs([{ date: '2026-08-03', bedTime: '23:30', wakeTime: '06:15' }]);
        assert.deepEqual(r, [{ date: '2026-08-03', bedTime: '23:30', wakeTime: '06:15' }]);
    });

    test('不正な行だけを除外し、他は残す', () => {
        const r = filterValidSleepLogs([
            { date: '2026-08-01', bedTime: '23:00', wakeTime: '06:00' },
            { date: '2026-08-02', bedTime: '', wakeTime: '06:00' },   // 時刻欠落
            null,
            { date: '', bedTime: '23:00', wakeTime: '06:00' },        // 日付欠落
            { date: '2026-08-03', bedTime: '00:00', wakeTime: '07:00' }
        ]);
        assert.equal(r.length, 2);
        assert.deepEqual(r.map(s => s.date), ['2026-08-01', '2026-08-03']);
    });

    test('同一日付は先頭のみ採用する', () => {
        const r = filterValidSleepLogs([
            { date: '2026-08-03', bedTime: '23:00', wakeTime: '06:00' },
            { date: '2026-08-03', bedTime: '01:00', wakeTime: '09:00' }
        ]);
        assert.equal(r.length, 1);
        assert.equal(r[0].bedTime, '23:00');
    });

    test('配列でない入力は空配列', () => {
        assert.deepEqual(filterValidSleepLogs(null), []);
        assert.deepEqual(filterValidSleepLogs('x'), []);
    });
});
...
describe('computeSleepStats', () => {
    ...
    test('直近7日の平均睡眠時間・最新・不足日数を返す', () => {
        const logs = [
            { date: '2026-08-01', bedTime: '23:00', wakeTime: '06:00' }, // 7.0
            { date: '2026-08-02', bedTime: '00:00', wakeTime: '06:00' }, // 6.0
            { date: '2026-08-03', bedTime: '23:30', wakeTime: '06:15' }  // 6.75(前夜)
        ];
        const s = computeSleepStats(logs, TODAY);
        assert.equal(s.nights, 3);
        assert.equal(s.avgHours, 6.58);
        assert.equal(s.latest.date, '2026-08-03');
        assert.equal(s.latest.hours, 6.75);
        assert.equal(s.shortNights, 1); // 目標6.5未満は8/02の6.0だけ
    });

    test('窓の外の記録は集計しない', () => {
        const logs = [
            { date: '2026-07-01', bedTime: '22:00', wakeTime: '08:00' }, // 窓外
            { date: '2026-08-03', bedTime: '23:00', wakeTime: '06:00' }
        ];
        const s = computeSleepStats(logs, TODAY);
        assert.equal(s.nights, 1);
        assert.equal(s.avgHours, 7);
    });

    test('就寝時刻が安定していればばらつきが小さく、乱れていれば大きい', () => {
        const steady = computeSleepStats([
            { date: '2026-08-01', bedTime: '23:00', wakeTime: '06:00' },
            { date: '2026-08-02', bedTime: '23:10', wakeTime: '06:00' },
            { date: '2026-08-03', bedTime: '22:50', wakeTime: '06:00' }
        ], TODAY);
        const erratic = computeSleepStats([
            { date: '2026-08-01', bedTime: '21:00', wakeTime: '06:00' },
            { date: '2026-08-02', bedTime: '02:00', wakeTime: '06:00' },
            { date: '2026-08-03', bedTime: '23:30', wakeTime: '06:00' }
        ], TODAY);
        assert.ok(steady.bedTimeSpreadMin < 15, `安定側のばらつきが大きすぎる: ${steady.bedTimeSpreadMin}`);
        assert.ok(erratic.bedTimeSpreadMin > 100, `乱れ側のばらつきが小さすぎる: ${erratic.bedTimeSpreadMin}`);
        assert.equal(steady.avgBedTime, '23:00');
    });

    test('日付をまたぐ就寝の平均が正しい(23:30と00:30の平均は00:00)', () => {
        const s = computeSleepStats([
            { date: '2026-08-02', bedTime: '23:30', wakeTime: '06:00' },
            { date: '2026-08-03', bedTime: '00:30', wakeTime: '07:00' }
        ], TODAY);
        assert.equal(s.avgBedTime, '00:00');
    });

    test('記録が無ければnull', () => {
        assert.equal(computeSleepStats([], TODAY), null);
        assert.equal(computeSleepStats(null, TODAY), null);
        assert.equal(computeSleepStats([{ date: '2026-06-01', bedTime: '23:00', wakeTime: '06:00' }], TODAY), null);
    });
});
