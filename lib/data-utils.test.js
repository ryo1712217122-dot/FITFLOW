...
describe('renameExercisesInWorkouts', () => {
        ...
        return [
            {
                ...
                exercises: [
                    { name: 'アームカール', sets: [{ weight: 14, reps: 5 }] },
                    { name: 'レッグプレス', sets: [{ weight: 200, reps: 10 }] }
                ]
            },
            {
                id: 'w2', date: '2026-07-12',
                exercises: [{ name: 'アームカール', sets: [{ weight: 14, reps: 5 }] }]
            }
        ];
    }
    ...
    test('一致する種目名だけを改名し、改名した件数を返す', () => {
        const data = workouts();
        const count = renameExercisesInWorkouts(data, 'アームカール', 'ダンベルフライ');
        assert.equal(count, 2);
        assert.equal(data[0].exercises[0].name, 'ダンベルフライ');
        assert.equal(data[0].exercises[1].name, 'レッグプレス'); // 他の種目は変更しない
        assert.equal(data[1].exercises[0].name, 'ダンベルフライ');
    });

    test('一致する種目が無ければ何も変更せず0を返す(再実行しても冪等)', () => {
        const data = workouts();
        renameExercisesInWorkouts(data, 'アームカール', 'ダンベルフライ');
        const secondRun = renameExercisesInWorkouts(data, 'アームカール', 'ダンベルフライ');
