...
describe('computeFoodCategoryBreakdown', () => {
    const categoryKeys = [
        { key: 'ramen', calKey: 'ramenCalories' },
        { key: 'sweets', calKey: 'sweetsCalories' }
    ];

    test('指定した年月の記録だけをカテゴリ別に集計する', () => {
        const logs = [
            { date: '2023-02-01', ramen: true, ramenCalories: 500 },
            { date: '2023-02-15', ramen: true, ramenCalories: 600 },
            { date: '2023-02-20', sweets: true, sweetsCalories: 300 },
            { date: '2023-03-01', ramen: true, ramenCalories: 999 } // 対象月の外
        ];
        const result = computeFoodCategoryBreakdown(logs, categoryKeys, '2023-02');
        assert.deepEqual(result, {
            ramen: { count: 2, totalKcal: 1100 },
            sweets: { count: 1, totalKcal: 300 }
        });
    });

    test('kcal未入力の記録は件数だけ数え、totalKcalには加算しない', () => {
        const logs = [{ date: '2023-02-01', ramen: true, ramenCalories: null }];
        const result = computeFoodCategoryBreakdown(logs, categoryKeys, '2023-02');
        assert.deepEqual(result.ramen, { count: 1, totalKcal: 0 });
    });

    test('該当データが無くても、全カテゴリのキーを0件で返す', () => {
        const result = computeFoodCategoryBreakdown([], categoryKeys, '2023-02');
        assert.deepEqual(result, {
            ramen: { count: 0, totalKcal: 0 },
            sweets: { count: 0, totalKcal: 0 }
        });
    });
});
