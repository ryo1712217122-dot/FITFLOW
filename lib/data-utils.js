...
function computeMeasuredTdee(weightLogs, mealLogs, todayStr, {
    windowDays = 28, minWeightPoints = 8, minSpanDays = 14, minMealDays = 10,
    ...
    if (points.length < minWeightPoints) return null;
...
// 減量ロードマップの各節目を返す。現在の体重と実効アンダーカロリーから、
// stepDays刻みでtotalDaysまでの予測体重を算出する(既定は2週間ごと・3ヶ月分)。
//
// 保存済みのマイルストーン(weightStart/weight1Month/weight3Month)ではなく、
// 選択中の減量ペースからその場で計算する。ペースを変えた瞬間に全段が追従し、
// 「保存された計画」と「画面の予測」が食い違わないようにするため。
// 戻り値: [{ days, label, weight, delta, isMajor }] (先頭は現在=days 0)
function computeWeightRoadmap(latestWeight, dailyDeficit, { stepDays = 14, totalDays = 90 } = {}) {
    const start = Number(latestWeight);
    if (!isFinite(start) || start <= 0) return [];

    const labelFor = (days) => {
        if (days === 0) return '現在';
        if (days % 30 === 0) return `${days / 30}ヶ月後`;
        if (days % 7 === 0) return `${days / 7}週間後`;
        return `${days}日後`;
    };

    const points = [];
    for (let days = 0; days <= totalDays; days += stepDays) {
        points.push(days);
    }
    // 刻みで割り切れず終端が欠ける場合(例: 14日刻みで90日)、最終日を必ず含める
    if (points[points.length - 1] !== totalDays) points.push(totalDays);

    return points.map(days => {
        const weight = projectWeightAfterDays(start, dailyDeficit, days);
        return {
            days,
            label: labelFor(days),
            weight,
            delta: Math.round((weight - start) * 10) / 10,
            // 1ヶ月単位の節目と終端は強調表示する
