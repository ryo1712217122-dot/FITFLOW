...
// "HH:MM" を 0〜1439 の分に変換する。形式が不正ならnull。
...
// 就寝時刻と起床時刻から睡眠時間(時間、小数2桁)を求める。
// 起床が就寝以前なら日付をまたいだものとして24時間を足す(23:30就寝→6:15起床 = 6.75時間)。
// 就寝と起床が同時刻の場合は0ではなく24時間になってしまうため、不正としてnullを返す。
...
// 就寝時刻を「18:00からの経過分」に置き換える。
// 就寝時刻のばらつきを普通に平均・標準偏差すると、23:30と00:30が「23時間差」に
// なってしまう(時刻は循環量なので算術平均が使えない)。現実の就寝は20時〜翌4時に
// 収まるので、18:00を原点に取り直せば循環をまたがずに素直な統計量が使える。
function bedTimeToAnchoredMinutes(bedTime) {
    const bed = parseTimeToMinutes(bedTime);
    if (bed === null) return null;
    return (bed - 18 * 60 + 1440) % 1440;
}

// 睡眠ログ(1日1件、{date, bedTime, wakeTime})の検証と正規化。
// 他のログと同じく、不正な行は全体を捨てずにその行だけ除外する。
// 日付は「起床日」に紐づける(前夜23:30に寝て今朝6:15に起きた睡眠は今日の記録)。
// 同一日付は先頭のみ採用する(1日1件前提。クラウド往復で重複しても集計が二重にならないように)。
function filterValidSleepLogs(data) {
    if (!Array.isArray(data)) return [];
    const seen = new Set();
    const result = [];
    data.forEach(s => {
        const duration = s ? computeSleepDuration(s.bedTime, s.wakeTime) : null;
        if (!s || typeof s.date !== 'string' || !s.date || duration === null) {
            console.warn('☁️ 不正な睡眠ログをスキップしました:', s);
            return;
        }
        if (seen.has(s.date)) return;
        seen.add(s.date);
        result.push({ date: s.date, bedTime: s.bedTime.trim(), wakeTime: s.wakeTime.trim() });
    });
    return result;
}

// 直近days日の睡眠を集計する。ブリーフィングと画面表示の両方がこれを使う。
// 戻り値:
//   nights          集計できた日数
//   avgHours        平均睡眠時間
//   latest          最新の1件 {date, bedTime, wakeTime, hours}
//   shortNights     targetHours未満だった日数
//   bedTimeSpreadMin 就寝時刻の標準偏差(分)。生活リズムの安定度を表す
//   avgBedTime      平均就寝時刻 "HH:MM"
// 集計できる日が無ければnull。
function computeSleepStats(sleepLogs, todayStr, { days = 7, targetHours = 6.5 } = {}) {
    if (!Array.isArray(sleepLogs) || !todayStr) return null;
    const DAY_MS = 24 * 60 * 60 * 1000;
    const today = new Date(todayStr + 'T00:00:00');
    if (isNaN(today.getTime())) return null;
    const windowStart = new Date(today.getTime() - (days - 1) * DAY_MS);

    const valid = filterValidSleepLogs(sleepLogs);
    const inWindow = [];
    valid.forEach(s => {
        const d = new Date(s.date + 'T00:00:00');
        if (isNaN(d.getTime()) || d < windowStart || d > today) return;
        inWindow.push(Object.assign({}, s, { hours: computeSleepDuration(s.bedTime, s.wakeTime) }));
    });
    if (inWindow.length === 0) return null;
    inWindow.sort((a, b) => (a.date < b.date ? -1 : a.date > b.date ? 1 : 0));

    const hours = inWindow.map(s => s.hours);
    const avgHours = Math.round((hours.reduce((a, b) => a + b, 0) / hours.length) * 100) / 100;

    const anchored = inWindow.map(s => bedTimeToAnchoredMinutes(s.bedTime)).filter(v => v !== null);
    let bedTimeSpreadMin = null;
    let avgBedTime = null;
    if (anchored.length > 0) {
        const meanAnchored = anchored.reduce((a, b) => a + b, 0) / anchored.length;
        const variance = anchored.reduce((sum, v) => sum + (v - meanAnchored) ** 2, 0) / anchored.length;
        bedTimeSpreadMin = Math.round(Math.sqrt(variance));
        const absMin = Math.round(meanAnchored + 18 * 60) % 1440;
        avgBedTime = `${String(Math.floor(absMin / 60)).padStart(2, '0')}:${String(absMin % 60).padStart(2, '0')}`;
    }
    ...
    return {
        ...
        shortNights: hours.filter(h => h < targetHours).length,
        bedTimeSpreadMin,
        avgBedTime
    };
}
...
// なお平均摂取の窓だけは「昨日まで」で、今日は含めない(理由は下の集計部のコメント参照)。
function computeMeasuredTdee(weightLogs, mealLogs, todayStr, {
    ...
    //
    // 今日は集計から外す(窓は windowStart 〜 昨日 の windowDays-1 日間)。
    // 今日の食事記録は「まだ食べ終わっていない」ため、朝や昼にアプリを開けばほぼ必ず
    // 未完成の値になる。実測TDEE = 平均摂取 + 7700×減少ペース なので、未完成の1日が
    // そのまま平均を押し下げ、実測TDEEと目標摂取カロリーを恒常的に低く見せていた
    // (実データで11日中の今日1日が200kcalだったとき、平均2088→1917で171kcalの過小評価)。
    // 体重の回帰は今日を含めたままでよい(体重測定は1回で完結し未完成になりようがない)。
    //
    ...
    const intakeEnd = new Date(today.getTime() - DAY_MS);
    const intakeWindowDays = Math.max(1, windowDays - 1);
    const intakes = [];
    const seenMealDates = new Set();
    mealLogs.forEach(m => {
        if (!m || !m.date || seenMealDates.has(m.date)) return;
        const d = new Date(m.date + 'T00:00:00');
        if (isNaN(d.getTime()) || d < windowStart || d > intakeEnd) return;
    ...
    // 記録率の分母も「今日を除いた日数」に揃える(記録しようのない今日を分母に残すと、
    // 毎日きちんと記録していても記録率が100%に届かない)
    const mealCoverage = intakes.length / intakeWindowDays;
